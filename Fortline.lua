...
local function GetAndEquipAR()
	...
	local EquippedAR = Character:FindFirstChild("AR")

	if EquippedAR and EquippedAR:IsA("Tool") then
		return EquippedAR
	end
	...
	local AR = Backpack:FindFirstChild("AR")

	if AR and AR:IsA("Tool") then
		Humanoid:EquipTool(AR)

		task.wait()

		local Equipped = Character:FindFirstChild("AR")
