...
--// Get and equip Sniper
local function GetAndEquipSniper()
	...
	local EquippedSniper = Character:FindFirstChild("Sniper")

	if EquippedSniper and EquippedSniper:IsA("Tool") then
		return EquippedSniper
	end
	...
	local Sniper = Backpack:FindFirstChild("Sniper")

	if Sniper and Sniper:IsA("Tool") then
		Humanoid:EquipTool(Sniper)

		for _ = 1, 20 do
			task.wait(0.05)

			local Equipped = Character:FindFirstChild("Sniper")
...
local function PlayGunfireSound(Weapon)
	...
	for _, Sound in ipairs(Sounds) do
		...
		if string.find(LowerName, "fire")
			...
			or string.find(LowerName, "sniper")
...
local function FireAtAllPlayers()
	local Weapon = GetAndEquipSniper()
